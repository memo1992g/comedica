...
export const postTcReportService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<TcReportRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<TcReportResponseI>> => {
  ...
  return res as BackofficeApiResponse<TcReportResponseI>;
};
