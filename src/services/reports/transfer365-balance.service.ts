...
export const postTransfer365BalanceService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<Transfer365BalanceRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<Transfer365BalanceData>> => {
  ...
  const res = await customAuthFetch(
    `${API_URL}/reportes/transfer365-balance`,
    {
      method: "POST",
  ...
  return res as BackofficeApiResponse<Transfer365BalanceData>;
};
