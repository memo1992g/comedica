...
export const postFinancialCorrespondentsReportService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<FinancialCorrespondentsReportRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<FinancialCorrespondentsReportResponseI>> => {
  ...
  const res = await customAuthFetch(
    `${API_URL}/reportes/correspondent`,
    {
      method: "POST",
  ...
  return res as BackofficeApiResponse<FinancialCorrespondentsReportResponseI>;
};
