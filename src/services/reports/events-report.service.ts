...
export const postEventsReportService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<EventsReportRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<EventsReportResponseI>> => {
  ...
  const res = await customAuthFetch(`${API_URL}/reportes/events-report`, {
    method: "POST",
    body: JSON.stringify(data),
    headers,
  });

  return res as BackofficeApiResponse<EventsReportResponseI>;
};
