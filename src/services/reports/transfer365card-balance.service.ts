...
export const postTransfer365CardBalanceService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<Transfer365CardBalanceRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<Transfer365CardBalanceData>> => {
  ...
  const res = await customAuthFetch(
    `${API_URL}/reportes/transfer365-card-balance`,
    {
      method: "POST",
  ...
  return res as BackofficeApiResponse<Transfer365CardBalanceData>;
};
