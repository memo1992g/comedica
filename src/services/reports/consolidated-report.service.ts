...
export const postConsolidatedReportService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<ConsolidatedReportRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<ConsolidatedReportResponseI>> => {
  ...
  const res = await customAuthFetch(
    `${API_URL}/reportes/consolidated-report`,
    {
      method: "POST",
  ...
  return res as BackofficeApiResponse<ConsolidatedReportResponseI>;
};
