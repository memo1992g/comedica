...
import type {
  EventsReportRequestI,
  EventsReportResponseI,
} from "@/interfaces/reports";
import { throwActionError } from "@/lib/error-handle";
import { postEventsReportService } from "@/services/reports/events-report.service";
...
export const eventsReportAction = async (
  filters: EventsReportRequestI,
  pagination?: Partial<BackofficeApiPaginationParams>,
): Promise<ActionResult<EventsReportResponseI>> => {
  try {
    const { requestId } = getActionDefaults();
    const baseRequest: BackofficeApiRequestI<EventsReportRequestI> = {
      request: filters,
    ...
    const res = await postEventsReportService({
      data: baseRequest,
    });
    ...
    return {
      data: null,
      errors: true,
      errorMessage: res.result?.message || "Error al obtener reporte de eventos",
    };
  } catch (error: unknown) {
