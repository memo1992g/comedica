...
export const postMhReportService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<MhReportRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<MhReportResponseI>> => {
  ...
  const res = await customAuthFetch(`${API_URL}/reportes/mh`, {
    method: "POST",
    body: JSON.stringify(data),
    headers,
  });

  return res as BackofficeApiResponse<MhReportResponseI>;
};
