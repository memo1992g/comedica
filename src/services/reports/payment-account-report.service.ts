...
export const postPaymentAccountReportService = async ({
  data,
  noToken = true,
}: {
  data: BackofficeApiRequestI<PaymentAccountReportRequestI>;
  noToken?: boolean;
}): Promise<BackofficeApiResponse<PaymentAccountReportResponseI>> => {
  ...
  const res = await customAuthFetch(`${API_URL}/reportes/payment-account`, {
    method: "POST",
    body: JSON.stringify(data),
    headers,
  });

  return res as BackofficeApiResponse<PaymentAccountReportResponseI>;
};
